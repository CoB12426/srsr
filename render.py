...
if __name__ == '__main__':
    ...
    try:
        ...
        if not os.path.exists("output/"+'test'):
            ...
            os.makedirs("output/"+'test'+"/frames")
            os.makedirs("output/"+'test'+"/video")
    ...
    with torch.no_grad():
        ...
        for i in tqdm_:
            ...
            if i==0:
                ...
                writer = cv2.VideoWriter("output/"+'test'+"/video/" + 'test' + ".mp4",
                                         cv2.VideoWriter_fourcc(*'mp4v'), 60, image_size)

            if i%1==0:
                cv2.imwrite("output/"+'test/frames/'+ f"_{i:03d}_lq.jpg", x)
                cv2.imwrite("output/"+'test/frames/'+ f"_{i:03d}_sr.jpg", out)
